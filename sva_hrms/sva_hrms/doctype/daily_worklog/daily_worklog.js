...
frappe.ui.form.on("Daily Worklog", {
    ...
    setup: function (frm) {
        ...
        frm.fields_dict["timesheet_detail"].grid.get_field("project").get_query = function () {
            return {
                filters: {
    ...
    // onload: function (frm) {
    //     if (frm.doc.__islocal && frm.doc.timesheet_detail) {
    ...
    //         $.each(frm.doc.timesheet_detail || [], function (i, row) {
    ...
    //                     frm.refresh_fields("timesheet_detail");
    //                     frm.save();
    //                     flag = false;
    //                 }
    ...
    //         frm.fields_dict["timesheet_detail"].grid.toggle_enable("billing_hours", false);
    //         frm.fields_dict["timesheet_detail"].grid.toggle_enable("is_billable", false);
    //     }
    ...
    exchange_rate: function (frm) {
        $.each(frm.doc.timesheet_detail, function (i, d) {
    ...
    set_dynamic_field_label: function (frm) {
        ...
        if (frm.doc.timesheet_detail.length > 0) {
            frm.set_currency_labels(
            ...
            let timesheet_detail_grid = frm.fields_dict.timesheet_detail.grid;
            $.each(
                ["base_billing_rate", "base_billing_amount", "base_costing_rate", "base_costing_amount"],
                function (i, d) {
                    if (frappe.meta.get_docfield(timesheet_detail_grid.doctype, d))
                        timesheet_detail_grid.set_column_disp(d, frm.doc.currency != base_currency);
                }
            );
        }
        frm.refresh_fields();
    ...
    set_route_options_for_new_task: (frm) => {
        let task_field = frm.get_docfield("timesheet_detail", "task");
...
frappe.ui.form.on("Timesheet Detail", {
    timesheet_detail_remove: function (frm) {
        calculate_time_and_amount(frm);
    },
    ...
    timesheet_detail_add: function (frm, cdt, cdn) {
...
//     let tl = frm.doc.timesheet_detail || [];
...
function set_project_in_timelog(frm) {
    if (frm.doc.parent_project) {
        $.each(frm.doc.timesheet_detail || [], function (i, item) {
